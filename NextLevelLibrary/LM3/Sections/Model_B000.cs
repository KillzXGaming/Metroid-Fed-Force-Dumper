...
namespace NextLevelLibrary.LM3
{
    public class ModelFormat : IFormat
    {
        ...
        public class MeshInfo
        {
            ...
            /// 
            /// </summary>
            internal uint UVLayerBufferPointer { get; set; }
            ...
            public ModelFormat ModelFile;

            public MeshInfo(ModelFormat modelFile, Mesh mesh) {
            ...
            public MaterialTable.MaterialData GetMaterial()
            {
               return MaterialTable.Instance.CreateMaterial(ModelFile, this);
            }
        }

        public class MaterialData
        {
            public MaterialTable.MaterialData Material;
        ...
        public class Mesh
        {
            ...
            public uint MaterialKindHash; //Shader program hash for loading shader/material params
        ...
        public ModelFormat(ChunkFileEntry fileChunk, SkeletonFormat skeleton)
        {
            ...
            for (int i = 0; i < chunkList.Count; i++)
            {
                ...
                switch (chunk.Type)
                {
                    ...
                    case ChunkType.MeshInfo:
                        ...
                        {
                            var mesh = new MeshInfo(this, meshHeaders[m]);
                            meshes.Add(mesh);
                        }
                        ...
                        using (var reader = new FileReader(vertexPointerChunk.Data, true))
                        {
                            for (int m = 0; m < numMeshes; m++)
                            {
                                ...
                                if (reader.BaseStream.Length >= reader.Position + 4)
                                    mesh.UVLayerBufferPointer = reader.ReadUInt32();
                            }
                        }
                        break;
                  ...
                  //  default:
                    //    throw new Exception($"Unsupported model chunk {chunk.ChunkType}!");
                }
            }
            ...
            //Read materials
            if (MaterialTable.Instance != null)
            {
                foreach (var mesh in meshes)
                    mesh.Material.Material = mesh.GetMaterial();
            }
