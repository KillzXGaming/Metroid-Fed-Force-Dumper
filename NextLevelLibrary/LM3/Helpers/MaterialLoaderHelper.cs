...
namespace NextLevelLibrary.LM3
{
    public class MaterialLoaderHelper
    {
        ...
        /// </summary>
        /// <returns></returns>
        static List<uint[]> GetLookupPointers(List<ModelFormat.MeshInfo> meshes, Stream lookupStream)
        {
            List<uint[]> pointers = new List<uint[]>();
            using (var lookupReader = new FileReader(lookupStream, true))
            {
                for (int i = 0; i < meshes.Count; i++) {
                    byte numPointers = meshes[i].Header.MaterialLookupPointerCount;
        ...
        public static List<TreeNodeModel> CreateMaterialChunkList(Stream materialStream, Stream lookupStream, List<ModelFormat.MeshInfo> meshes)
        {
            ...
            List<TreeNodeModel> meshNodes = new List<TreeNodeModel>();
            using (var reader = new FileReader(materialStream, true))
            {
                for (int i = 0; i < meshes.Count; i++)
                {
                    TreeNodeModel meshNode = new TreeNodeModel(Hashing.GetString(meshes[i].Header.Hash));
                    ...
                    for (int j = 0; j < pointers.Length; j++)
                    {
                        ...
                        var stream = new SubStream(materialStream, pointers[j], size);

                        DataTreeNode matNode = new DataTreeNode($"{meshes[i].Material.Name}_{pointers[j]}_{j}", stream);
        ...
        public static void ParseMaterials(Stream materialStream, Stream lookupStream, List<ModelFormat.MeshInfo> meshes)
        {
          //  SetMaterialsHacky(materialStream, lookupStream, meshes);
            ...
            using (var reader = new FileReader(materialStream, true))
            {
                for (int i = 0; i < meshes.Count; i++) {
                    ...
                    uint[] pointers = materialPointers[i];
                    uint hash = meshes[i].Header.MaterialHash;
                    string materialPreset = Hashing.GetString(hash);

                    mat.Name = materialPreset;
                    mat.Lookup = pointers.ToList();
                    ...
                    for (int j = 0; j < matPointers.Count; j++)
                    {
                        if (j + 1 < matPointers.Count)
                        {
                            ...
                            while (reader.Position < matPointers[j] + size)
                            {
                                ...
                                if (Hashing.TextureHashCache.Contains(hashCheck))
                                {
        ...
        static void SetMaterialsHacky(Stream materialStream, Stream lookupStream, List<ModelFormat.MeshInfo> meshes)
        {
            List<uint[]> materialPointers = GetLookupPointers(meshes, lookupStream);
                    ...
                    for (int j = 0; j < matPointers.Count; j++)
                    {
                        ...
                        if (matPointers[j] % 448 == 0 || globalPointers.Contains(matPointers[j] + 448))
                        {
                            ...
                            if (texhash == 0x81800000)
                            {
                                ...
                                if (Hashing.TextureHashCache.Contains(texhash))
                                {
                                ...
                                if (Hashing.TextureHashCache.Contains(texhash2))
                                {
                        ...
                        else
                        {
                            ...
                            if (a == 0x81800000)
                            {
                                ...
                                if (Hashing.TextureHashCache.Contains(hash1))
                                {
